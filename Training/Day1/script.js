...
$.ajax({
  ...
  success: function (result) {
    ...
    $(obj).each(function (index, item) {
      let row = $("<tr>");
      row.html(`<td>${item.login}</td>
    <td>${item.id}</td>
    <td>${item.node_id}</td>
    <td><a href="${item.url}" target="_blank">${item.url}</a></td>
    <td><img src="${item.avatar_url}" alt="${item.login}" width="50"></td>
    <td>${item.description}</td>`);
      $("#orgTable tbody").append(row);
    });
  },
});
// $.get("https://api.github.com/users/hadley/orgs",function(data,status){
//   let obj = data;
//     $(obj).each(function (index, item) {
//       let row = $("<tr>");
//       row.html(`<td>${item.login}</td>
...
//   let tableBody = document.querySelector("#orgTable tbody");
